...
# 剥离 Markdown 代码块标记（```json ... ``` / ``` ... ```）——细节 1：格式微小偏差不误杀合法响应
_MD_FENCE_RE = re.compile(r"^```(?:json)?\s*|\s*```$")
...
class LlmClient:
    ...
    def _extract_json(self, raw: str) -> str:
        """从 LLM 响应提取 JSON：剥离代码块/首尾空白。why：DeepSeek 偶发把 JSON 包在 ```json 外层，
        直接 json.loads 会误杀合法响应——先清洗再解析，提高成功率（细节 1）。"""
    ...
    def chat_json(self, system: str, user: str, *, schema: type[BaseModel]) -> tuple[BaseModel, LlmUsage]:
        ...
        for attempt in range(s.max_retries):
            ...
            if attempt < s.max_retries - 1:
                time.sleep(s.retry_backoff * (2**attempt))  # 指数退避 1s/3s/9s
