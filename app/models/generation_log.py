...
class GenerationLog(TimestampMixin, Base):
    ...
    status: Mapped[str] = mapped_column(String(16), nullable=False)  # success/validation_failed/error
    ai_confidence: Mapped[float] = mapped_column(Float, nullable=False, default=1.0)  # 校验通过=1.0、失败=0.0
    usage: Mapped[dict | None] = mapped_column(JSON)  # {prompt_tokens, completion_tokens, total_tokens}
