...
4. 每条用例 assertions 至少 1 条：接口定义了响应 schema 时校验其字段；未定义响应 schema 时，
   用 {"path": "status_code", "op": "eq", "value": expected_status}，不得编造接口定义中不存在的字段。
...
assertions 为数组且至少 1 条，每项 {"path": 响应字段路径, "op": "eq"|"ne"|"contains"|"exists", "value": 期望值}。
