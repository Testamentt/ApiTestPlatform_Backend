        ...
        [
            {
                ...
                "request_body": {"required": True, "content": {"application/json": {"schema": {"$ref": "#/components/schemas/User"}}}},
            }
        ],
        ...
        [
            {
                ...
                "method": "POST",
                "operation_id": "createUser",
                "request_body": {"content": {"application/json": {"schema": {"allOf": [
                    {"type": "object", "properties": {"a": {"type": "string"}}, "required": ["a"]},
                    {"$ref": "#/components/schemas/Base"},
                ]}}}},
            }
        ],
        schemas={"Base": {"type": "object", "properties": {"b": {"type": "integer"}}, "required": ["b"]}},
    )
    ...
    doc = _swagger([
        {
            ...
            "method": "POST",
            "operation_id": "createUser",
            "request_body": {"content": {"application/json": {"schema": {"oneOf": [
    ...
    doc = _swagger([
        {
            ...
            "method": "POST",
            "operation_id": "createUser",
            "request_body": {"content": {"application/json": {"schema": {"type": "array", "items": {"type": "string", "enum": ["a", "b"]}}}}},
        }
    ])
    contract = parse_openapi(doc).operation_contracts["createUser"]
...
def test_request_change_only_request_hash():
    ...
    h2 = parse_openapi(_swagger([_users_op(parameters=[
        {"name": "limit", "in": "query", "schema": {"type": "integer"}, "required": True}])])).operation_hashes["listUsers"]
...
def test_status_code_change_changes_response_hash():
    ...
    d1 = _swagger([{**base, "responses": {"200": {"content": {"application/json": {"schema": {"type": "object"}}}}}}])
    d2 = _swagger([{**base, "responses": {"202": {"content": {"application/json": {"schema": {"type": "object"}}}}}}])
...
def test_hash_ignores_description():
    ...
    h1 = parse_openapi(_swagger([_users_op(description="获取用户列表")])).operation_hashes["listUsers"]
    h2 = parse_openapi(_swagger([_users_op(description="获取所有用户")])).operation_hashes["listUsers"]
    assert h1 == h2
...
def test_hash_changes_on_constraint_tightening():
    ...
    h2 = parse_openapi(_swagger([
        {**base, "request_body": {"content": {"application/json": {"schema": {
...
def test_hash_detects_property_removed():
    d1 = _swagger([_body_op({"type": "object", "properties": {"name": {"type": "string"}, "age": {"type": "integer"}}})])
...
def test_hash_detects_enum_removed_in_property():
    d1 = _swagger([_body_op({"type": "object", "properties": {"status": {"type": "string", "enum": ["active", "pending"]}}})])
    d2 = _swagger([_body_op({"type": "object", "properties": {"status": {"type": "string", "enum": ["active"]}}})])
    h1 = parse_openapi(d1).operation_hashes["createUser"]
    h2 = parse_openapi(d2).operation_hashes["createUser"]
    assert h1["request"] != h2["request"]
...
def test_hash_keeps_ignoring_description_inside_property():
    ...
    d1 = _swagger([_body_op({"type": "object", "properties": {"name": {"type": "string", "description": "旧"}}})])
    d2 = _swagger([_body_op({"type": "object", "properties": {"name": {"type": "string", "description": "新"}}})])
    h1 = parse_openapi(d1).operation_hashes["createUser"]
    h2 = parse_openapi(d2).operation_hashes["createUser"]
    assert h1 == h2
...
def test_warning_broken_ref():
    doc = _swagger(
        [_users_op(parameters=[{"name": "x", "in": "query", "schema": {"$ref": "#/components/schemas/Nope"}}])],
        schemas={},
    )
    api = parse_openapi(doc)
...
def test_warning_external_ref_preserved():
    doc = _swagger([
        {**_users_op(), "request_body": {"content": {"application/json": {"schema": {"$ref": "./common.yaml#/User"}}}}}
    ])
    api = parse_openapi(doc)
...
def test_malformed_schema_is_tolerated():
    # 宽容解析：畸形结构不抛，正常返回
    doc = _swagger([{**_users_op(), "request_body": {"content": {"application/json": {"schema": {"type": "bogus"}}}}}])
    api = parse_openapi(doc)
