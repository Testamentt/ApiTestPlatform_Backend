...
def test_identical_when_hashes_equal():
    d = diff_operations(["a"], _hashes(["a"]), {"a": _contract()}, ["a"], _hashes(["a"]), {"a": _contract()})
    assert d.identical and d.changed == [] and d.added == [] and d.removed == []


def test_added_removed_detected():
    old_ids, old_h, old_c = ["a", "b", "c"], _hashes(["a", "b", "c"]), {"a": _contract(), "b": _contract(), "c": _contract()}
    new_ids, new_h, new_c = ["b", "c", "d"], _hashes(["b", "c", "d"]), {"b": _contract(), "c": _contract(), "d": _contract()}
...
def test_hash_change_detected():
    old = ["b", "c"]
    old_h = _hashes(["b", "c"])
    new_h = {"b": {"request": "2", "response": "1"}, "c": {"request": "1", "response": "1"}}  # 仅 b 变
    d = diff_operations(old, old_h, {x: _contract() for x in old}, old, new_h, {x: _contract() for x in old})
    assert d.changed == ["b"]


def test_hash_version_mismatch_marks_all_changed():
    # 哈希算法升级：公共接口保守全标变更（不重建旧快照）
    d = diff_operations(["a", "b"], _hashes(["a", "b"]), {"a": _contract(), "b": _contract()},
                        ["a", "b"], _hashes(["a", "b"]), {"a": _contract(), "b": _contract()},
...
def _diff_breaking(old_c, new_c):
    return diff_operations(["op"], _hashes(["op"]), {"op": old_c},
                           ["op"], _hashes(["op"], req="2"), {"op": new_c})
...
def test_contract_missing_is_breaking():
    ...
    d = diff_operations(["op"], _hashes(["op"]), {"op": None},
                        ["op"], _hashes(["op"], req="2"), {"op": _contract(["x"])})
    assert d.breaking_changed == ["op"]
